...
"""
 ...
 7. Integrated Mistral API for multi-LLM experimentation.
 """
import os
import json
import logging
...
class Composition:
    def __init__(self, title: str, melody: Optional[MelodyIdea] = None, harmony: Optional[HarmonyProgression] = None, rhythm: Optional[RhythmPattern] = None, arrangement: Optional[Arrangement] = None, score: Optional[str] = None, audio_path: Optional[str] = None):
        self.title = title
        self.melody = melody
...
class CompositionOrchestrator:
    def __init__(self, api_keys: Dict[str, str], llm_choice: str = "gemini"):
    ...
    def _get_llm(self):
        ...
        elif self.llm_choice == "mistral":
            mistral_api_key = self.api_keys.get("mistral")
            if not mistral_api_key:
                raise ValueError("Mistral API key not provided in api_keys.")
            return MistralAI(api_key=mistral_api_key, model="mistral-medium")  # Or another Mistral model
        else:
            raise ValueError(f"Invalid llm_choice: {self.llm_choice}. Choose 'gemini' or 'mistral'.")

    def setup_agents(self):
        llm = self._get_llm()
        self.melody_agent = MelodyAgent(llm)
        self.harmony_agent = HarmonyAgent(llm)
        self.rhythm_agent = RhythmAgent(llm)
        self.arrangement_agent = ArrangementAgent(llm)
...
def example_usage():
    api_keys = {
        ...
        "mistral": "YOUR_MISTRAL_API_KEY"  # Replace with your actual Mistral API key
    }

    # To use Gemini:
    orchestrator_gemini = CompositionOrchestrator(api_keys, llm_choice="gemini")
    composition_gemini, response_gemini = orchestrator_gemini.compose_music(
        style="classical",
        mood="happy",
        title="Spring"
        # You could also pass constraints here, e.g.: {"max_length": 10, "key": "C major"}
    )
    print("--- Gemini Composition Process ---")
    print(response_gemini.response)
    print("\n--- Gemini Final Composition ---")
    print(json.dumps(composition_gemini.to_dict(), indent=2))
    if composition_gemini.audio_path and os.path.exists(composition_gemini.audio_path):
        print(f"\nGemini Audio file generated: {composition_gemini.audio_path}")
    else:
        print("\nNo Gemini audio file was generated.")

    print("\n" * 2)

    # To use Mistral:
    orchestrator_mistral = CompositionOrchestrator(api_keys, llm_choice="mistral")
    composition_mistral, response_mistral = orchestrator_mistral.compose_music(
        style="classical",
        mood="happy",
        title="Spring"
        # You could also pass constraints here, e.g.: {"max_length": 10, "key": "C major"}
    )
    print("--- Mistral Composition Process ---")
    print(response_mistral.response)
    print("\n--- Mistral Final Composition ---")
    print(json.dumps(composition_mistral.to_dict(), indent=2))
    if composition_mistral.audio_path and os.path.exists(composition_mistral.audio_path):
        print(f"\nMistral Audio file generated: {composition_mistral.audio_path}")
    else:
        print("\nNo Mistral audio file was generated.")
